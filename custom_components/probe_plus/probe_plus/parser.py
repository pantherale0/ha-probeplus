...
class ProbePlusData:
    ...
    relay_battery: float = 0
    relay_voltage: float = 0
    relay_status: int = 0
    probe_battery: float = 0
    probe_voltage: float = 0
    probe_temperature: float = 0
    probe_rssi: float = 0
...
class ParserBase:
    ...
    def parse_data(self, data: bytearray):
        ...
        if len(data) == 9 and data[0] == 0x00 and data[1] == 0x00:
            ...
            temp_bytes = data[4:6]
            self.state.probe_temperature = ((struct.unpack(">H", temp_bytes)[0] * 0.0625) - 50.0625) / 100
